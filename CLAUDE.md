...
1. **Pre-computed FZP baseline**: The `fzp-zoning.csv` file contains pre-computed expected units at FZP heights (`fzp_expected_units_low` and `fzp_expected_units_high`). These are generated by `scripts/precompute_units.py`.
