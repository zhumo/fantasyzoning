#!/usr/bin/env python3
"""Step 19: Calculate expected units using FZP model"""
import sys
...
def run():
    ...
    parcel_z = np.zeros(len(df))
    for field in PARCEL_FIELDS:
        if field in df.columns:
            parcel_z += PROB_WEIGHTS[field] * _to_numeric_series(df[field]).values

    env = _to_numeric_series(df['Env_1000_Area_Height']).values
    sdb_env = _to_numeric_series(df['SDB_2016_5Plus_EnvFull']).values
    zoning_dr = _to_numeric_series(df['Zoning_DR_EnvFull']).values
    ...
    df['fzp_expected_units_low'] = prob_low * units
    df['fzp_expected_units_high'] = prob_high * units
    ...
    return {'input': initial_count, 'output': len(df), 'total_low': total_low, 'total_high': total_high}


if __name__ == '__main__':
    r = run()
    print(f"[{STEP_DIR.name}] {r['input']:,} -> {r['output']:,}")
    print(f"  Total expected units (low): {r['total_low']:,.0f}")
    print(f"  Total expected units (high): {r['total_high']:,.0f}")
